                    ...
                    <form method="get" action="{{ route('events.index') }}">
                        <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                                ...
                                <div class="col-md-10">
                                    <div class="dt-action-buttons text-xl-end text-lg-start text-md-end text-start d-flex align-items-center justify-content-end flex-md-row flex-column mb-3 mb-md-0">
                                        <div class="dataTables_filter">
                                            <label>
                                                <input type="text" value="{{ request('keyword') }}" name="keyword" class="form-control" placeholder="Search by name...">
                                            </label>
                                        </div>
                            ...
                            <table class="datatables table dataTable no-footer dtr-column" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                                ...
                                @endif
                                @foreach ($thisDatas as $eventKey => $event)
                                    <tr>
                                        <td>{{ $eventKey + 1 }}</td>
                                        <td>{{ $event->title }}</td>
                                        <td>
                                            <span class="badge @if ($event->status) bg-label-primary @else bg-label-secondary @endif me-1">
                                                @if ($event->status)
                                                    {{ __('Active') }}
                                                @else
                                                    {{ __('Inactive') }}
