                    ...
                    <form method="get" action="{{ route('teams.index') }}">
                        <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                                        ...
                                        <div class="dt-buttons btn-group flex-wrap">
                                            <div class="btn-group">
                                                ...
                                                @if(hasPermission('/' . strtolower($title) . '/*', 'put'))
                                                    <a class="btn add-new btn-primary text-white" href="{{ route('teams.create') }}">
                            ...
                            <table class="datatables table dataTable no-footer dtr-column" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                                ...
                                @endif
                                @foreach ($thisDatas as $teamKey => $team)
                                    <tr>
                                        <td>{{ $teamKey + 1 }}</td>
                                        <td>{{ $team->name }}</td>
                                        <td>{{ $team->position }}</td>
                                        <td>
                                            <span class="badge @if ($team->status) bg-label-primary @else bg-label-secondary @endif me-1">
                                                @if ($team->status)
                                                    {{ __('Active') }}
                                                @else
                                                    {{ __('Inactive') }}
                                            ...
                                            @if(hasPermission('/' . strtolower($title) . '/*', 'put') || hasPermission('/' . strtolower($title) . '/*', 'delete'))
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @if(hasPermission('/' . strtolower($title) . '/*', 'put'))
                                                            <a class="dropdown-item" href="{{ route('teams.edit', $team->id) }}"><i class="ti ti-pencil me-1"></i>{{ __('Edit') }}</a>
                                                        @endif
                                                        @if(hasPermission('/' . strtolower($title) . '/*', 'delete'))
                                                            <a href="#" class="dropdown-item delete-button" data-bs-toggle="modal" data-actionurl="{{ route('teams.destroy', $team->id) }}" data-bs-target="#deleteModal">
